...
const CollectionsPage = () => {
    ...
    const fetchCollections = async () => {
        try {
            setLoading(true)
            const response = await fetch('/api/sale-collection/getallcollections')
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.header}>
                ...
                <Link href="/sale-collections/new" className={styles.createButton}>
                    <Plus size={20} />
                    New Collection
                </Link>
            </div>
                    ...
                    collections?.sort((a, b) => (a?.order ?? Infinity) - (b?.order ?? Infinity))?.map((collection) => (
                        <div key={collection._id} className={styles.collectionCard}>
                            ...
                            <div className={styles.actionButtons}>
                                <button 
                                    onClick={() => router.push(`/sale-collections/${collection._id}`)}
